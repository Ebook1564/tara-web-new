...
export default function IntelligenceSamplePage() {
  ...
  const handleRequestSample = (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      try {
        const fetchRes = await fetch(`/api/authors/admin/uploads/${name.trim()}`);
        if (fetchRes.ok) {
  ...
  const handleRecordClaim = async (sample: Sample) => {
    try {
      await fetch("/api/authors/claims", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: name,
  ...
  const handleDownload = async (sample: Sample) => {
    ...
    try {
      const res = await fetch(`/api/authors/admin/uploads/${name.trim()}?id=${sample.id}`);
      if (res.ok) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-start">
          ...
          <div className="relative lg:sticky lg:top-28">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
              Get Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-blue-600">Authors Preview</span>
            </h1>
                  ...
                  <form className="space-y-6" onSubmit={handleRequestSample}>
                    <div>
                      <label htmlFor="name" className="block text-sm font-medium text-slate-700 mb-2">
                        Authors Name (as per LinkedIn) <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        id="name"
                    ...
                    >
                      ...
                      {isChecking ? (
                        ...
                        <>
                          <span className="relative z-10 tracking-wide drop-shadow-sm">Get Your Preview</span>
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-2 px-8 py-4 rounded-xl text-lg font-bold transition-all duration-200
                          ${downloadingId === sample.id
                            ...
                            : "bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white shadow-lg hover:shadow-xl hover:shadow-purple-500/30 transform hover:-translate-y-1"
                          }`}
                      >
                        {downloadingId === sample.id ? (
                          <>
